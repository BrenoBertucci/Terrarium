-- What Vermilion's ground and fences cost a frame: both kits off, on, on, off
-- -- a palindrome, so warm-up and heat charge both sides alike.
return function(game)
  local out = assert(os.getenv("DS_PROBE_DIR"))
  local file = assert(io.open(out .. "/vermilion_cost_probe.log", "w"))
  ...
  local Ground, Fence, Mesher = lib.require("LavenderGroundKit"), lib.require("FenceKit"), lib.require("ChunkMesher")
  local function measure(enabled)
    Ground.ENABLED, Fence.ENABLED = enabled, enabled
    Mesher.invalidate()
    enter(19, 7)
    settle(900)
    ...
    log(("kits %s  mean %.2f ms a step"):format(enabled and "ON " or "OFF", sum / n * 1000))
  end
  measure(false) measure(true) measure(true) measure(false)
  Ground.ENABLED, Fence.ENABLED = nil, nil
  file:close()
  love.event.quit()
end
