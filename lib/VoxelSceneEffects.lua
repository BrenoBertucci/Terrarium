...
local function billboardMatrix(px, py, y, mirror, yaw)
  ...
  local b = (okFP and FirstPerson) and FirstPerson.cardBlend() or 0
  local halfW = 8
  local m = Mat4.translate(px + halfW, y, py + halfW)

  if b > 0 then
    m = Mat4.mul(m, Mat4.rotateY(FirstPerson.cardYaw(px + halfW, py + halfW) * b))
  else
    ...
    m = Mat4.mul(m, Mat4.rotateX(Voxel.angle - math.pi / 2))
  end
...
-- Helper function to get the facing direction for a pose, accounting for
-- first-person camera. Used by STADIUM_OVERWORLD_MODELS patcher.
local function viewFacing(p)
  local facing = p.facing or "down"
  if FirstPerson and FirstPerson.cardBlend() > 0.5 then
    if p.isPlayer then
...
-- Draw character sprites/billboards. This function is patched by
-- STADIUM_OVERWORLD_MODELS to inject Stadium model rendering.
-- The patcher expects this exact signature and structure.
local function drawCast(posed, me, yaw)
...
local function drawGhost(p, yaw)
  ...
  local frame, mirror = frameFor(def, p.facing, p.phase, p.flip, yaw)
...
function VoxelScene.render(state, w, h, vw, vh, paletteFor)
  ...
  -- its own call, so this is only the overworld.)

  drawCast(posed, me, yaw)
