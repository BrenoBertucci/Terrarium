...
- **One DINAMICA bug fixed along the way, found while building the
  ...
  four command chips finished preparing; if any one of them failed, the
  whole function reported failure and the flat box drew on top of what
  was already there. The four chips are now prepared before the message
  panel touches the screen. (The PR's other two fixes -- the advance
  arrow's position and the `!` cell of the B2W2 name font -- both lived
  in the art path this repository dropped in 1.37.0-beta, so neither
  came across.)
