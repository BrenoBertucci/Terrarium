...
| the **TREES** options row | VOXEL / CLASSIC — how trees are built in the diorama. VOXEL grows a tree on every round-tree site: a visible trunk, forking branches, a canopy of leaf clumps with ambient occlusion baked in, a fringe of alpha leaf cards, four species (oak, pine, birch, willow) mixed across the wood — and the crown bends in the wind while the bole stays planted. CLASSIC is the Gen 1 ball: one outline-hulled sphere carved from the tileset art, identical on every cell, free because it rides in the chunk mesh — and for the same reason it cannot move in the wind. Flipping it rebuilds the map's meshes over the next frames |
